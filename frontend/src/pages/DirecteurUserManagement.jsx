...
const getFieldSx = (roleFilter) => {
  const accentColor = roleFilter === 'ETUDIANT' ? C.green : C.blue;
  return {
...
const DirecteurUserManagement = () => {
  ...
  const [filters, setFilters]       = useState({
    search: '', role: roleFromUrl, statut: '',
  });
  const [filterOptions, setFilterOptions] = useState({ roles: [], statuts: [] });
  ...
  const fetchFilterOptions = async () => {
    try {
      const res = await api.get('/users/directeur/filter-options');
      setFilterOptions({
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const res = await api.get('/users/directeur', { params: { page: page + 1, limit: rowsPerPage, ...clean } });
  ...
  const getRoleDescription = () => {
    if (filters.role === 'ENSEIGNANT') return 'Gérez les enseignants de votre établissement';
    if (filters.role === 'ETUDIANT') return 'Gérez les étudiants de votre établissement';
    return 'Gérez les enseignants et étudiants de votre établissement';
  };
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 4, p: 3, borderRadius: 3, background: '#fff', border: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <Box sx={{
            ...
            background: filters.role === 'ETUDIANT' ? '#E8F8F5' : (filters.role === 'ENSEIGNANT' ? '#DBEAFE' : '#EFF6FF'),
      ...
      <Card sx={{
        ...
        <Box sx={{ 
          height: 3, 
          background: filters.role === 'ETUDIANT'
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
              <Box sx={{ 
                ...
                background: filters.role === 'ETUDIANT' ? '#ECFDF5' : '#F0F7FF', 
                display: 'flex', alignItems: 'center', justifyContent: 'center', 
                boxShadow: filters.role === 'ETUDIANT' ? `0 3px 10px ${C.green}30` : `0 3px 10px ${C.blue}30`
              }}>
                <FilterList sx={{ color: filters.role === 'ETUDIANT' ? C.green : C.blueD, fontSize: 16 }} />
              </Box>
                      ...
                      <Card
                        ...
                        sx={{
                          ...
                          '&::before': {
                            ...
                            background: user.type_utilisateur === 'ETUDIANT'
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5, mb: 2 }}>
                            <Box sx={{ position: 'relative', flexShrink: 0 }}>
                              <Box sx={{
                                ...
                                borderRadius: '12px',
                                background: user.type_utilisateur === 'ETUDIANT' ? '#D1FAE5' : '#BFDBFE',
      ...
      >
        {selectedUser && (() => {
          ...
          const detailAccent = filters.role === 'ETUDIANT' ? C.green : C.blue;
          const detailBgLight = filters.role === 'ETUDIANT' ? '#ECFDF5' : C.blueL;
          const detailBgGrad = filters.role === 'ETUDIANT' ? 'linear-gradient(135deg, #ECFDF5 0%, #D1FAE5 100%)' : `linear-gradient(135deg, ${C.blueL} 0%, #D6EEFF 100%)`;
...
export default DirecteurUserManagement;
