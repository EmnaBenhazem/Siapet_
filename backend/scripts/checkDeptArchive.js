...
async function run() {
  ...
  const { rows } = await client.query(`SELECT column_name, data_type FROM information_schema.columns WHERE table_name='departement' ORDER BY ordinal_position`);
  console.log('departement cols:');
